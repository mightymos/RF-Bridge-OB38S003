...
void enter_DefaultMode_from_RESET(void)
{
	...
	//*****************************************
    // FIXME: default is SYSCLK_DIV_8 (0x3), so keep in mind that SYSCLK_DIV_1 must be explicitly set
    ...
    // FIXME: we set this when we call dosniffing() anyway
    TH0 = 0x0B;
	...
	//***********************************************************************/
    // FIXME: many of these are the default (i.e. 0) so do not need to be explicitly set
    ...
    // FIXME: equivalent to init_timer1()
    TMOD |= T1M__MODE2;
    ...
    // FIXME: this would not have been running in the first place anyway, unless we reinitialize sometime after startup
	// Stop Timer
	TMR2CN0 &= ~(TR2__BMASK);


	// FIXME: stop is the default setting anyway
	PCA0CN0 = CR__STOP;
	...
	// FIXME: bit cleared is the default anyway
    // this clears the autoreload register select (but this only applies to PWM modes, we are doing capture?)
	 ...
	 ***********************************************************************/
    // FIXME: equivalent to enable_capture_interrupt()
    ...
    // FIXME: equivalent to enable_serial_interrupt() enable_global_interrupts() and need to handle timer2 interrupt enable
    IE |= ET2__ENABLED;
    IE |= ES0__ENABLED;
    IE |= EA__ENABLED;
}
