...
/// Declares a variable to be located in 8051 BDATA (bit-addressable) space.
/// Not supported by SDCC ! Expect a compiler error here !
 ...
 * register of the 8051.  This cannot be used to make any arbitrary memory
 * location into an SFR.  The _address_ must map to a real 16-bit SFR in the
 * memory map in little endian byte order.
 *****************************************************************************/
#define SI_SFR16(name, address) __sfr16 __at ((address+1U)<<8) | address) name
 ...
 * To define the interrupt function implementation, use @ref SI_INTERRUPT.
 *****************************************************************************/
#define SI_INTERRUPT_PROTO(name, vector) void name (void) __interrupt (vector)
 ...
 * use @ref SI_INTERRUPT_USING.
 *****************************************************************************/
#define SI_INTERRUPT_PROTO_USING(name, vector, regnum)                        \
             void name (void) __interrupt (vector) __using (regnum)
 ...
 * @param return_type The data type of the function return value
 * (void, int, etc).
 * @param parameter_list Full C function parameter list (or "void") (type and
 * name), must include surrounding parentheses.
 *
 * This macro defines a function to be reentrant.
 *
 * You must specify the _return_type_ which is the type of the function.  It
 * can be `void` or any other C type or typedef.  The _parameter_list_ argument
 * is the list of function parameters.  It can be `(void)` or else it must be
 * a parameter data type and name surrounded by parentheses.  It can also be
 * multiple parameters but they must be enclosed in parentheses and separated
 * by commas.
 *
 * __Example__
 *
 ...
 * the function prototype, use @ref SI_REENTRANT_FUNCTION_PROTO.
 *****************************************************************************/
#define SI_REENTRANT_FUNCTION(name, return_type, parameter_list) \
  return_type name parameter_list __reentrant
 ...
 * define the function implementation, use @ref SI_REENTRANT_FUNCTION.
 *****************************************************************************/
#define SI_REENTRANT_FUNCTION_PROTO(name, return_type, parameter_list) \
  return_type name parameter_list __reentrant
 ...
 * the function prototype, use @ref SI_FUNCTION_PROTO_USING.
 *****************************************************************************/
#define SI_FUNCTION_USING(name, return_value, parameter_list, regnum)         \
             return_value name parameter_list __using (regnum)
 ...
 * define the function implementation, use @ref SI_FUNCTION_USING.
 *****************************************************************************/
#define SI_FUNCTION_PROTO_USING(name, return_value, parameter_list, regnum) \
             return_value name parameter_list __using (regnum)
 ...
 *****************************************************************************/
#define SI_LOCATED_VARIABLE_NO_INIT(name, vartype, memseg, address)          \
             vartype memseg name __at (address)
...
// Generic pointer memory segment constants.
#define SI_GPTR                   ///< Generic pointer indeterminate type.
#define SI_GPTR_MTYPE_DATA  0x40  ///< Generic pointer for DATA segment.
#define SI_GPTR_MTYPE_IDATA 0x40  ///< Generic pointer for IDATA segment.
#define SI_GPTR_MTYPE_BDATA 0x40  ///< Generic pointer for BDATA segment.
#define SI_GPTR_MTYPE_PDATA 0x60  ///< Generic pointer for PDATA segment.
...
typedef union SI_GEN_PTR
{
  void * ptr;       ///< 3-byte generic pointer of unspecified type.
  uint8_t u8[3];    ///< 3-byte generic pointer as 3 unsigned 8-bit integers.
  GPTR_t gptr;      ///< 3-byte generic pointer as pointer structure
} SI_GEN_PTR_t;
